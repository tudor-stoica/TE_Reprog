...
def data_load(args, test):
    ...
    file_dirs_sor = args.s_dset_path
    file_dirs_test = args.test_dset_path
    time_len = 10
    num_class = args.class_num

    dsets["source"] = cl.data_conca(file_dirs_sor, time_len, [5], num_class)
    # dsets["source"] = DealDataset(dsets["source"], 4)
    if args.trte == "val":
        dsize = dsets["source"].shape[0]
        tr_size = int(0.1*dsize)
        tr_size_class = int(0.9*tr_size)
        # print(dsize, tr_size, dsize - tr_size)
        source_tr = np.zeros((1, 10, 51))
        source_te = np.zeros((1, 10, 51))
    ...
    dsets["test"] = cl.data_conca(file_dirs_test, time_len, [5], num_class)
    dsets["test"] = DealDataset(dsets["test"], 4)
    if test == 1:
        shuffle = False
    else:
        shuffle = True
    dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, shuffle=shuffle, num_workers=args.worker, drop_last=False)
    dset_loaders["source_te"] = DataLoader(dsets["source_te"], batch_size=train_bs, shuffle=shuffle, num_workers=args.worker, drop_last=False)
    dset_loaders["test"] = DataLoader(dsets["test"], batch_size=train_bs, shuffle=shuffle, num_workers=args.worker, drop_last=False)
...
def train_source(args):
    ...
    #     netF = network.VGGBase(vgg_name=args.net).cuda()  #特征提取网络pretrain

    netF = network.MyAlexNet(resnet_name=args.net)
    netF = netF.cuda()
    # netF.in_features = 5 * 10 * 48
    netF.in_features = 5 * 5 * 24
    ...
    while iter_num < max_iter:
        ...
        classifier_loss = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth)(outputs_source, labels_source.long())
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset=='VISDA-C':
                ...
                acc_s_te, acc_list = cal_acc(dset_loaders['source_te'], netF, netB, netC, False)
                log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%'.format(args.name_src, iter_num, max_iter, acc_s_te)
...
def test_target(args, dset_loaders):
    # dset_loaders = data_load(args, 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='TE', choices=['VISDA-C', 'office', 'office-home', 'office-caltech', 'TE'])
    ...
    # folder = './data/'
    folder = r'D:/Python/DA_based_FD/matlab_pro/'
    args.s_dset_path = folder + names[args.s] + '_new/'
    args.test_dset_path = folder + names[args.t] + '_new/'
